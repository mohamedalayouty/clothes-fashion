...
const Cart = ({decreament, increament, totalPrice, productCart, deleteProductCart, total}) => {
...
return (
    <div className='flex justify-evenly mx-4'>
        <div className='w-3/4 mt-4 flex flex-col'>
            {productCart.length === 0 ? (
                        ...
                        <Link to="/shop">
                        <Button  size='sm' className='bg-green-700'>SHOP NOW</Button>
                        </Link>
                        </div>
            ) : 
            (  <div>
            {productCart.map((product, index)=> (
                <div key={index} className='lg:flex lg:flex-row lg:gap-8 flex-col me-4 gap-4 items-center font-bold'>
                <button><img className='mb-3 lg:w-52 w-auto' src={product.img} alt="" loading='lazy'/></button>
                ...
                <Button onClick={() => decreament(product)} 
                className='bg-white text-[#3d4b5e] text-lg'>-</Button>
                <h1>{product.num}</h1>
                <Button onClick={() => increament(product)} 
                className='bg-white text-[#3d4b5e] text-lg'>+</Button>
                <h1 className='text-[#3d4b5e]'>{parseInt(product.price) * product.num} $</h1>
    ...
    <div className='h-1/2 bg-gray-200 mt-4 py-4 px-4 flex flex-col me-4'>
        ...
        <div className='flex justify-between text-lg'>
            <h1 className='font-bold text-[#3d4b5e] my-1 '>{totalPrice}</h1>
