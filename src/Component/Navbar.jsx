...
const Navbar = ({ productCart, profileUser, signOut, setrender, render }) => {
  useEffect(() => {
    setrender(true);
  }, [render]);
  return (
    <div className="flex justify-between bg-gray-200 p-2">
      <div className="ms-12 flex items-center me-4">
        <img src={img} alt="" />
      </div>
      <div className="flex items-center font-bold text-gray-500 cursor-pointer">
        <Link
          to="/"
          className="mr-4 no-underline focus:underline decoration-red-900 underline-offset-4 text-gray-500 focus:text-black"
        >
          Home
        </Link>
        <Link
      ...
      </div>
      {
        ...
        bg-red-500 rounded-full w-1/2 text-center text-white text-sm"
            >
              {productCart.length}
            </h1>
            <CiLock className="mr-4 size-6 cursor-pointer" />
          </Link>
          {localStorage.login === "member" || localStorage.login === "Admin" ? (
            <Menu>
              <MenuHandler>
                <Avatar
                  variant="circular"
            ...
            <Link onClick={() => signOut()} to="/sign in">
              <ImExit className=" text-indigo-700 size-5 cursor-pointer" />
            </Link>
          )}
        </div>
      }
